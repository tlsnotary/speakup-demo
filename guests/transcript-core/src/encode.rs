...
//! JSON path → the flat public `u32` table the guest verifies.
//!
...
//! reaches the guest — then resolves the path to node indices, extracts the
//! public claims (method, target, `Host` value, status), assembles the
//! words, and finally cross-checks its own output against [`crate::verify`]
//! natively.
...
use crate::{
    ...
    TABLE_CAP_WORDS, VERSION, W_CLAIM_MODE, W_EXPECT_SLEN, W_EXPECT_SOFF, W_HOST_SLEN,
    W_HOST_SOFF, W_METHOD_END, W_METHOD_SLEN, W_METHOD_SOFF, W_N_NODES, W_N_PATH, W_NODES_OFF,
    W_PATH_OFF, W_REASON_END, W_REASON_START, W_RECV_LEN, W_REQ_CL_IDX, W_REQ_HDRS_OFF,
    W_REQ_HEAD_END, W_REQ_HOST_IDX, W_REQ_N_HDRS, W_RESP_CL_IDX, W_RESP_HDRS_OFF, W_RESP_HEAD_END,
    W_RESP_N_HDRS, W_SENT_LEN, W_STATUS, W_STR_BYTES, W_STR_OFF, W_TARGET_END, W_TARGET_SLEN,
    W_TARGET_SOFF, W_VALUE_NODE, W_VERSION,
};
...
pub struct Encoded {
    ...
    pub status: u16,
    /// Content span of the disclosed value, in `recv` coordinates.
...
fn parse_path(path: &str) -> Result<Vec<Comp<'_>>, String> {
    if path.is_empty() {
        return Ok(Vec::new());
    }
...
/// method, target, `Host`, and status, whose JSON response body has a value
/// at `path`" as the guest's flat public table. With `expect = None` the
/// value's bytes are disclosed; with `Some(v)` the guest instead asserts
/// they equal `v` and reveals only the 0/1 flag (a mismatched `v` encodes
/// fine — the proof then legitimately fails).
pub fn encode(
    sent: &[u8],
    recv: &[u8],
    table: &SpanTable,
    ...
    // === resolve the path over the (decoded-coordinate) node table ===
    let nodes = &json.nodes;
    let dbody = &recv[head_end..]; // Content-Length framing: decoded == raw
    let comps = parse_path(path)?;
    let mut resolved: Vec<(u32, u32, &[u8])> = Vec::new(); // (is_index, idx, key bytes)
    let mut cur = 0usize;
    for comp in &comps {
        let nd = &nodes[cur];
        match comp {
    ...
    let nodes_off = resp_hdrs_off + 4 * resp.headers.len();
    let path_off = nodes_off + 4 * nodes.len();
    let str_off = path_off + 4 * resolved.len();

    // Packed string region: method ++ target ++ host value ++ key comps.
    ...
    for (i, n) in nodes.iter().enumerate() {
        // Decoded-body → `recv` source coordinates (identical layout under
        // Content-Length framing, shifted by the head).
    ...
    let value_bytes = &recv[value_start..value_start + value_len];
